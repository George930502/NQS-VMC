...
def parallel_metropolis_hastings_sampler(model, n_samples_per_chain, n_chains, n_spins, burn_in_steps, step_interval, device='cpu'):
    ...
    with torch.no_grad():
        ...
        print("Parallel Metropolis-Hastings sampling complete.")
...
def local_energy_batch(model, samples, qubit_hamiltonian, device='cpu'):
    ...
    E_loc = torch.zeros(n_samples, dtype=torch.cfloat, device=device)
    ...
    with torch.no_grad():
        ...
        for term, coeff in qubit_hamiltonian.terms.items():
            ...
            phase = torch.ones(n_samples, dtype=torch.cfloat, device=device)
...
def stochastic_reconfiguration_update(model, samples, qubit_ham, lr, reg, device='cpu'):
    ...
    for i in range(n_samples):
        ...
        O_list.append(grad_vec.to(torch.cfloat))
    ...
    S = S.to(torch.cfloat)
    S += reg * torch.eye(n_p, dtype=torch.cfloat, device=device)

    # Compute force vector F (vectorized)
    local_es = local_energy_batch(model, samples, qubit_ham, device=device).to(dtype=torch.cfloat)
